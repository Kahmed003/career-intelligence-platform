...
/*
Migration: 20260721012100_create_opportunity_matching_intelligence.sql
Purpose: Create explainable profile-to-opportunity match intelligence.
*/
begin;
...
select
    ...
    case
        ...
        when c.preference_level = 'avoid'
             and eval.match_state = 'matched'
            then -c.weight
...
cross join lateral (
    select
        case c.criterion_type
            ...
            when 'industry' then
                case
                    when op.industry is null then 'not_evaluable'
                    when lower(btrim(op.industry)) = lower(btrim(c.criterion_value))
                        then 'matched'
                    else 'not_matched'
                end
            ...
            when 'location' then
                case
                    ...
                    when lower(op.location_text) like '%' || lower(btrim(c.criterion_value)) || '%'
                        then 'matched'
                    else 'not_matched'
                end
            ...
            when 'country' then
                case
                    ...
                    when lower(op.country_code) = lower(btrim(c.criterion_value))
                        then 'matched'
                    else 'not_matched'
                end
            ...
            when 'work_mode' then
                case
                    ...
                    when lower(op.work_mode) = lower(btrim(c.criterion_value))
                        then 'matched'
                    else 'not_matched'
                end
            ...
            when 'employment_type' then
                case
                    ...
                    when lower(op.employment_type) = lower(btrim(c.criterion_value))
                        then 'matched'
                    else 'not_matched'
                end

            when 'opportunity_type' then
                case
                    when op.opportunity_type is null then 'not_evaluable'
                    when lower(op.opportunity_type) = lower(btrim(c.criterion_value))
                        then 'matched'
                    else 'not_matched'
                end
            ...
            when 'organization' then
                case
                    ...
                    when c.criterion_value ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
                         and op.organization_id::text = lower(c.criterion_value)
                        then 'matched'
                    when org.legal_name is not null
                         and lower(btrim(org.legal_name)) = lower(btrim(c.criterion_value))
                        then 'matched'
                    else 'not_matched'
                end
            ...
            when 'sponsorship' then
                case
                    ...
                    when lower(op.visa_sponsorship) = lower(btrim(c.criterion_value))
                        then 'matched'
                    else 'not_matched'
                end
        ...
        case c.criterion_type
            when 'industry' then
                format(
                    'Opportunity industry is %s; criterion is %s.',
                    coalesce(op.industry, 'unknown'),
                    c.criterion_value
                )
            when 'location' then
                format(
                    'Opportunity location is %s; criterion is %s.',
                    coalesce(op.location_text, 'unknown'),
                    c.criterion_value
                )
            when 'country' then
                format(
                    'Opportunity country is %s; criterion is %s.',
                    coalesce(op.country_code, 'unknown'),
                    c.criterion_value
                )
            when 'work_mode' then
                format(
                    'Opportunity work mode is %s; criterion is %s.',
                    coalesce(op.work_mode, 'unknown'),
                    c.criterion_value
                )
            when 'employment_type' then
                format(
                    'Opportunity employment type is %s; criterion is %s.',
                    coalesce(op.employment_type, 'unknown'),
                    c.criterion_value
                )
            when 'opportunity_type' then
                format(
                    'Opportunity type is %s; criterion is %s.',
                    coalesce(op.opportunity_type, 'unknown'),
                    c.criterion_value
                )
            when 'organization' then
                format(
                    'Opportunity organization is %s; criterion is %s.',
                    coalesce(org.legal_name, op.organization_id::text, 'unknown'),
                    c.criterion_value
                )
            when 'sponsorship' then
                format(
                    'Opportunity sponsorship policy is %s; criterion is %s.',
                    coalesce(op.visa_sponsorship, 'unknown'),
                    c.criterion_value
                )
            else
                format(
...
comment on view public.opportunity_preference_evaluations is
'Criterion-level explainable evaluation of active career preference profiles against owner-matched opportunities.';
...
with base as (
    ...
    cross join lateral (
        select
            case
                when p.minimum_compensation is null
                    then 'not_requested'
                when op.compensation_min is null
                     and op.compensation_max is null
                    then 'unknown'
                when p.compensation_currency is null
                     or op.compensation_currency is null
                    then 'unknown'
                when upper(p.compensation_currency) <> upper(op.compensation_currency)
                    then 'unknown'
                when p.compensation_period is not null
                     and op.compensation_period is not null
                     and p.compensation_period <> op.compensation_period
                    then 'unknown'
    ...
    cross join lateral (
        select
            case
                when coalesce(p.requires_sponsorship, false) = false
                    then 'not_required'
                when op.visa_sponsorship = 'yes'
                    then 'compatible'
                when op.visa_sponsorship = 'no'
                    then 'incompatible'
                when op.visa_sponsorship = 'case_by_case'
                    then 'case_by_case'
                else 'unknown'
...
agg as (
    select
        ...
        count(*) filter (
            where e.match_state <> 'not_evaluable'
        )::integer as evaluable_criteria_count,

        count(*) filter (
            where e.match_state = 'matched'
        )::integer as matched_criteria_count,

        count(*) filter (
            where e.preference_level = 'must_have'
              and e.match_state = 'not_matched'
        )::integer as must_have_failure_count,

        count(*) filter (
            where e.is_exclusion = true
              and e.match_state = 'matched'
        )::integer as exclusion_count,

        sum(
            case
                when e.match_state = 'not_evaluable'
                  or e.preference_level = 'neutral'
                    then 0
                else e.weight
            end
        )::numeric as total_evaluable_weight,

        sum(
            coalesce(e.weighted_contribution, 0)
        )::numeric as net_weighted_contribution,

        jsonb_agg(
            jsonb_build_object(
                'type', e.criterion_type,
                'value', e.criterion_value,
                'weight', e.weight,
                'preference_level', e.preference_level,
                'exclusion', e.is_exclusion,
                'match_state', e.match_state,
                'explanation', e.explanation
            )
            order by e.weight desc, e.criterion_type, e.criterion_value
        ) as criterion_explanations
...
select
    ...
    case
        when coalesce(a.total_evaluable_weight, 0) <= 0
            then null::numeric
        else round(
            greatest(
                0::numeric,
                least(
                    100::numeric,
                    (
                        greatest(coalesce(a.net_weighted_contribution, 0), 0)
                        / a.total_evaluable_weight
                    ) * 100
                )
            ),
            2
    ...
    b.compensation_status,
    b.sponsorship_status,

    case
        when coalesce(a.exclusion_count, 0) > 0
            then 'excluded'
        when coalesce(a.must_have_failure_count, 0) > 0
            then 'disqualified'
        when b.sponsorship_status = 'incompatible'
            then 'sponsorship_conflict'
        when b.compensation_status = 'below_minimum'
            then 'compensation_conflict'
        when coalesce(a.total_evaluable_weight, 0) <= 0
    ...
    jsonb_build_object(
        ...
        'compensation', jsonb_build_object(
            'status', b.compensation_status,
            'profile_minimum', b.minimum_compensation,
            'profile_preferred', b.preferred_compensation,
            'profile_currency', b.profile_currency,
            'profile_period', b.profile_compensation_period,
            'opportunity_minimum', b.compensation_min,
            'opportunity_maximum', b.compensation_max,
            'opportunity_currency', b.opportunity_currency,
            'opportunity_period', b.opportunity_compensation_period
        ),
        'sponsorship', jsonb_build_object(
            'status', b.sponsorship_status,
...
from base b
left join agg a
  on a.preference_profile_id = b.preference_profile_id
 and a.opportunity_id = b.opportunity_id;

comment on view public.opportunity_match_scores is
'Explainable weighted opportunity match scores for active career preference profiles.';
